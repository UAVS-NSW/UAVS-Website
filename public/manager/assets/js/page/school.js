    ...
    table: {
        ...
        init(){
            var row_table = [ 
                ...
                { title: 'Email', name: 'name', orderable: true, }, 
                { title: 'Address', name: 'name', orderable: true, },  
    ...
    FullTab: {  
        Create: { 
            ...
            getVal(resource){ 
                ...
                var data_website    = $(`${resource}`).find('.data-website').val();  
                var data_address    = $(`${resource}`).find('.data-address').val();  

                // --Required Value  
                if (data_name == '') { required_data.push('Name valid.'); onPushData = false } 
                if (data_email == '') { required_data.push('Email valid.'); onPushData = false } 
                if (data_website == '') { required_data.push('Website valid.'); onPushData = false }   
                if (data_address == '') { required_data.push('Address valid.'); onPushData = false } 

                if (onPushData) { 
                    fd.append('data_image', data_image[0] ?? "null"); 
                    fd.append('data_name', data_name);  
                    fd.append('data_email', data_email);  
                    fd.append('data_website', data_website);  
        ...
        Update: { 
            setVal(resource, data){
                ...
                $(`${resource}`).find('.data-website').val(data.website);
...
(() => {
    ...
    View.FullTab.onPush("Delete", "#popup-delete", () => {
        ...
        View.FullTab.default();
        IndexView.helper.showToastProcessing('Process', 'Processing');
        Api.School.Delete($("#popup-delete").find('.data-id').val())
            .done(res => { 
                IndexView.helper.showToastSuccess('Success', 'Success');
                getData();
            })
